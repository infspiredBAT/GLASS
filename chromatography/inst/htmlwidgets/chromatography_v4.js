...
//        label_pos["intrex"]         = 120;
       ...
       function brushed() { redraw(); }
        ...
        function setBrush(start,end){
            ...
            //context.call(brush.move);
            //d3.select(this).transition().call(brush.move, [100,200]);
            //redraw();
        }
        ...
        function redraw()  {
            if (!d3.event.sourceEvent) {
                console.log("not source event");
                //return;
            }
            var s = d3.event.selection || width2Scale.range();
            //console.log(s);
            var t = s.map(width2Scale.invert, width2Scale)
            widthScale.domain(t);
        //     //console.log("redraw");
        ...
        //     }
            ...
            var w = t[1] - t[0]
            // var ext = d3.brushSelection(context);
            console.log(t);
            console.log(w);
            focus.selectAll("g").selectAll(".line_f").attr("d",line_fwd);
            focus.selectAll("g").selectAll(".line_r").attr("d",line_rev);
        ...
        function updateLine(data,base,rev){
            switch(base) {
                case "A": if(rev){var g = gLine_a_r;}else{var g = gLine_a;} var col = "#00A100"; break;
                case "C": if(rev){var g = gLine_c_r;}else{var g = gLine_c;} var col = "#2985EA"; break;
                case "G": if(rev){var g = gLine_g_r;}else{var g = gLine_g;} var col = "#6C6A6C"; break;
                case "T": if(rev){var g = gLine_t_r;}else{var g = gLine_t;} var col = "#EA2929"; break;
            }
            if(rev){var c = "line_r";var l = line_rev;}
            else{var c = "line_f";var l = line_fwd;}

            var line = g.selectAll("path").data(data); //UPDATE
  ...
  // 		         .attr("stroke-width",10).attr("stroke","brown").attr("opacity",0.2).attr("stroke-dasharray","1,3");
        ...
        function setCodingLabel(calls){
            ...
            aa_r.enter().append("text")
                .merge(aa_r).attr("class","peak_label short")
                .text(function(d){
//                    if   (d["ord_in_cod"] == 1) {return d["aa_ref"].toUpperCase()+""+d["codon"];}
            ...
            aa_s.enter().append("text") //aa_sample
                .merge(aa_s).attr("class","peak_label short aa_sample")
                .text(function(d){
            ...
            aa_m.enter().append("text")
                .merge(aa_m).attr("class","peak_label short aa_mut")
                .text(function(d){
